...
import { 
  TrendingUp, TrendingDown, Building2, CheckCircle2, 
  Clock, AlertTriangle, IndianRupee, BarChart3 
} from 'lucide-react';
  ...
  const fetchDashboard = async () => {
    ...
    try {
      const res = await fetch(
        `${API}/api/societies/${currentSociety.id}/maintenance/collection-dashboard?year=${year}&month=${month}`,
      ...
      {dashboard && (
        <>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-emerald-400 text-sm mt-1">
                      {dashboard.collection_percentage}% of billed
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-amber-400 text-sm mt-1">
                      {dashboard.pending_flats} flats pending
                    </p>
                  </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">
                    {dashboard.paid_flats} of {dashboard.paid_flats + dashboard.pending_flats} flats paid
                  </span>
                  <span className="text-cyan-400 font-medium">{dashboard.collection_percentage}%</span>
                </div>
                <Progress 
                  value={dashboard.collection_percentage} 
                  className="h-3 bg-slate-700"
                />
                <div className="flex gap-4 mt-4">
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                    <span className="text-sm text-slate-300">{dashboard.paid_flats} Paid</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Clock className="w-4 h-4 text-amber-400" />
                    <span className="text-sm text-slate-300">{dashboard.pending_flats} Pending</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <AlertTriangle className="w-4 h-4 text-red-400" />
                    <span className="text-sm text-slate-300">{dashboard.overdue_flats} Overdue</span>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <Card className="bg-slate-800/50 border-slate-700">
              <CardHeader>
                <CardTitle className="text-lg text-white">Monthly Breakdown ({year})</CardTitle>
              </CardHeader>
              <CardContent>
                          ...
                          {m.billed > 0 && (
                            <>
                              ...
                              <div 
                                ...
                                style={{ width: `${(m.pending / m.billed) * 100}%` }}
                              />
                            </>
                          )}
                        </div>
