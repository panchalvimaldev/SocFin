...
export default function PaymentEntry() {
  ...
  useEffect(() => {
    if (selectedFlat) {
      ...
      if (isAnnualPayment && selectedScheme) {
        fetchAnnualPreview();
      }
    } else {
      setBills([]);
  ...
  const fetchBills = async (flatId) => {
    try {
      const res = await fetch(
        `${API}/api/societies/${currentSociety.id}/maintenance/bills?flat_id=${flatId}&status=pending`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const calculateTotal = () => {
    ...
    if (isAnnualPayment && annualPreview) {
      return annualPreview.final_payable;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/societies/${currentSociety.id}/maintenance/payments`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  if (role !== 'manager') {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-slate-400">Only managers can record payments</p>
      </div>
    );
  }
            ...
            <div>
              ...
              <Select value={selectedFlat} onValueChange={setSelectedFlat}>
                <SelectTrigger className="bg-slate-900 border-slate-600 text-white mt-1">
                  <SelectValue placeholder="Choose flat" />
                </SelectTrigger>
                <SelectContent className="bg-slate-800 border-slate-700 max-h-[300px]">
                  {flats.map((flat) => (
                    <SelectItem key={flat.id} value={flat.id}>
                      {flat.flat_number} - {flat.wing} Wing ({flat.area_sqft} sqft)
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            {selectedFlat && (
              <>
                ...
                <div className="flex items-center gap-4 p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                  <Button
                    variant={!isAnnualPayment ? 'default' : 'outline'}
                    onClick={() => setIsAnnualPayment(false)}
                    className={!isAnnualPayment ? 'bg-cyan-600' : 'border-slate-600'}
                  >
                    Monthly Bills
                  </Button>
                  <Button
                    variant={isAnnualPayment ? 'default' : 'outline'}
                    onClick={() => setIsAnnualPayment(true)}
                    className={isAnnualPayment ? 'bg-emerald-600' : 'border-slate-600'}
                  >
                ...
                {!isAnnualPayment && (
                  <div>
                    ...
                    {bills.length === 0 ? (
                      <p className="text-slate-400 text-sm p-4 bg-slate-900/50 rounded-lg">
                        No pending bills for this flat
                      </p>
                    ) : (
                          ...
                          >
                            ...
                            <div className="flex-1">
                              <p className="text-white font-medium">
                                {bill.month}/{bill.year} - {bill.bill_period_type}
                              </p>
                            ...
                            <div className="text-right">
                              ...
                              {bill.paid_amount > 0 && (
                                <p className="text-xs text-emerald-400">
                                  ₹{bill.paid_amount.toLocaleString()} paid
                                </p>
                              )}
                            </div>
                          </div>
          ...
          <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-sm">
                      {selectedFlatData.wing} Wing • {selectedFlatData.area_sqft} sqft
                    </p>
                  </div>
                </div>
              </div>
            ...
            {isAnnualPayment && annualPreview && (
              <div className="space-y-3 p-4 bg-emerald-500/10 rounded-lg border border-emerald-500/30">
                ...
                {annualPreview.discount_amount > 0 && (
                  <div className="flex justify-between">
                    <span className="text-emerald-400">
                      Discount ({annualPreview.free_months} months free)
                    </span>
                    <span className="text-emerald-400">-₹{annualPreview.discount_amount.toLocaleString()}</span>
                  </div>
                )}
                <div className="flex justify-between pt-3 border-t border-emerald-500/30">
                  ...
                  <span className="text-emerald-400 font-bold text-xl">
                    ₹{annualPreview.final_payable.toLocaleString()}
                  </span>
                </div>
              </div>
            )}
            ...
            {!isAnnualPayment && selectedBills.length > 0 && (
              <div className="space-y-2">
                <div className="flex justify-between">
                  <span className="text-slate-400">Selected Bills</span>
            ...
            {customAmount && (
              <div className="p-3 bg-amber-500/10 rounded-lg border border-amber-500/30">
                <p className="text-amber-400 text-sm">Custom amount will override calculated total</p>
              </div>
            )}

            <Button
              onClick={handleSubmit}
              disabled={submitting || (!customAmount && calculateTotal() <= 0)}
              className="w-full bg-emerald-600 hover:bg-emerald-700"
              data-testid="record-payment-btn"
            >
              <Check className="w-4 h-4 mr-2" />
              {submitting ? 'Recording...' : `Record Payment ₹${calculateTotal().toLocaleString()}`}
            </Button>
          </CardContent>
        </Card>
      </div>
