...
export default function GenerateBills() {
  ...
  const fetchPreview = async () => {
    ...
    try {
      ...
      const res = await api.post(`/societies/${currentSociety.id}/maintenance/bills/preview`, body);
      setPreview(res.data);
    } catch (e) { console.error(e); } finally { setLoading(false); }
  };

  const generateBills = async () => {
    if (!window.confirm(`Generate ${billPeriodType} bills for ${billPeriodType === "monthly" ? `${months.find(m => m.value === month)?.label} ` : ""}${year}?`)) return;
    setGenerating(true);
    try {
      const body = {
        bill_period_type: billPeriodType,
      ...
      const res = await api.post(`/societies/${currentSociety.id}/maintenance/bills/generate`, body);
    ...
    <div className="space-y-6" data-testid="generate-bills-page">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="bg-slate-800/50 border-slate-700 lg:col-span-1">
          <CardHeader>
            <CardTitle className="text-lg text-white flex items-center gap-2">
              <Calculator className="w-5 h-5 text-cyan-400" />Bill Configuration
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {billPeriodType === "yearly" && schemes.length > 0 && (
              <>
                <div className="flex items-center justify-between pt-4 border-t border-slate-700">
                  <div><Label className="text-slate-300">Apply Discount Scheme</Label><p className="text-xs text-slate-500">For annual payments</p></div>
                ...
                {applyDiscount && (
                  <div>
                    ...
                    <Select value={selectedScheme} onValueChange={setSelectedScheme}>
                      <SelectTrigger className="bg-slate-900 border-slate-600 text-white mt-1"><SelectValue placeholder="Choose discount scheme" /></SelectTrigger>
                      <SelectContent className="bg-slate-800 border-slate-700">
            ...
            {settings && (
              <div className="p-3 bg-slate-900/50 rounded-lg border border-slate-700 mt-4">
                ...
                <p className="text-lg font-bold text-cyan-400">₹{settings.default_rate_per_sqft} <span className="text-sm font-normal text-slate-400">per sqft</span></p>
                <p className="text-xs text-slate-500 mt-1">Due on {settings.due_date_day}th of each month</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-slate-800/50 border-slate-700 lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-lg text-white flex items-center gap-2">
              <FileText className="w-5 h-5 text-emerald-400" />Bill Preview
            </CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              <div className="flex items-center justify-center h-48"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-cyan-400" /></div>
            ) : preview ? (
              <div className="space-y-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                    <div className="flex items-center gap-2 text-slate-400 text-xs mb-1"><Building2 className="w-3 h-3" />Total Flats</div>
                  ...
                  <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-700">
                    ...
                    <p className="text-2xl font-bold text-cyan-400">₹{preview.total_collection_before_discount.toLocaleString()}</p>
                    {preview.estimated_discount > 0 && <p className="text-xs text-emerald-400">-₹{preview.estimated_discount.toLocaleString()} discount</p>}
                  </div>
                  <div className="p-4 bg-emerald-500/10 rounded-lg border border-emerald-500/30">
                    <div className="flex items-center gap-2 text-emerald-400 text-xs mb-1"><Sparkles className="w-3 h-3" />Net Collection</div>
                    <p className="text-2xl font-bold text-emerald-400">₹{preview.total_collection_after_discount.toLocaleString()}</p>
                  </div>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-700">
                        ...
                        {preview.estimated_discount > 0 && <th className="text-right py-2 text-slate-400 font-medium">Discount</th>}
                    ...
                    <tbody>
                      {preview.bills_preview.slice(0, 10).map((bill, idx) => (
                        <tr key={idx} className="border-b border-slate-700/50">
                          ...
                          {preview.estimated_discount > 0 && <td className="py-2 text-right text-emerald-400">{bill.discount > 0 ? `-₹${bill.discount.toLocaleString()}` : "-"}</td>}
                  ...
                  {preview.bills_preview.length > 10 && <p className="text-center text-slate-500 text-sm py-2">...and {preview.bills_preview.length - 10} more flats</p>}
                </div>
                <div className="flex justify-end pt-4 border-t border-slate-700">
                  <Button onClick={generateBills} disabled={generating} className="bg-emerald-600 hover:bg-emerald-700" data-testid="generate-bills-btn">
                    <FileText className="w-4 h-4 mr-2" />{generating ? "Generating..." : `Generate ${preview.total_flats} Bills`}
                  </Button>
                </div>
              </div>
            ) : (
              <div className="text-center text-slate-400 py-12">Select billing period to preview</div>
            )}
          </CardContent>
        </Card>
      </div>
